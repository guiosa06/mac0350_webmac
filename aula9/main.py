...
def lista(index: int, request: Request, busca: str | None=''):
    alunos = buscar_alunos(busca)
    if(index*10>=len(alunos)):
        index-=1
    elif(index<0):
        index=0
    l = index*10
    r = l+10
    return templates.TemplateResponse(request, "lista.html", {"alunos": alunos[l:r], "index": index})
